...
## 6.1 Ollama REST APIの基礎

### 6.1.1 APIエンドポイント

Ollamaは起動時に自動的にREST APIサーバーを起動します（デフォルト: http://localhost:11434）。
...
# 実行
response = llm.invoke("MS-S1 Maxについて教えて")
print(response)
```
...
# 質問
query = "MS-S1 Maxのメモリ帯域幅は?"
result = qa_chain.invoke({"query": query})
print(result['result'])
```

## 6.4 OpenAI API 互換モード

### 6.4.1 エンドポイント

Ollamaは `/v1` エンドポイントでOpenAI API互換インターフェースを提供します。
...
```bash
# Continue.dev 拡張機能をインストール
# VSCode: Extensions → "Continue"

# 設定 (~/.continue/config.json)
{
  "models": [
    {
...
次章では、MS-S1 Max向けの徹底的なパフォーマンス最適化を学びます。

---

**前章へ**: [第5章 モデル管理とカスタマイズ](chapter05_model_management.md)
**次章へ**: [第7章 MS-S1 Max向けパフォーマンス最適化](chapter07_performance_optimization.md)
